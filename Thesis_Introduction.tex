...
Space robot locomotion may be of several types like manipulation, gripping, roving, drilling, sampling, flying, floating, diving or even climbing. On the other hand, a space robot's autonomy ranges from teleoperation to fully-autonomous, while other robots execute tasks on semi-autonomous operation mode.

In 1981, the first orbital robot, Canadarm, was placed in orbit with the goal to move payloads from the Space Shuttle to a final deployment position \cite{roboticsHandbook_chap55}. Later in 2001, Canadarm 2 was launched in order to assist astronauts during \acrshort{eva}, but also to maintain and build the ISS. In 2011, the humanoid robot Robonaut 2 was placed in the \acrshort{iss} to perform assisting or teleoperation tasks, for the sake of upgrading it to a future version in order to assist astronauts during \acrshort{eva}s. The Moon sampler Surveyor 3 was the first planetary robot to be launched (1967). From the 90s onwards, Mars has become the main exploratory target for surface robots. The \textit{Sojourner} rover, the twin Spirit and Opportunity \acrshort{mer} and the \acrshort{msl} have provided valuable scientific information and have enabled ground teams to design rovers which reach higher velocities and larger lifetimes. Recently, in 2021, the Perseverance rover landed on Mars taking on its belly a small robotic helicopter which accomplished the first powered, controlled flight on the thin atmosphere of Mars\footnote{https://www.space.com/mars-helicopter-ingenuity-first-flight-success (accessed on 23 of March 2026)}.
...
As it is stated in Table \ref{tab:roverAchievements}, launched planetary, autonomous rovers have achieved higher lifetimes as well as higher traverse speeds and travelled greater distances each mission goes by. Below, only rovers deployed in Mars are shown, but the \acrshort{cnsa} has also deployed two rovers on the Moon, - \textit{Yutu and Yutu-2} - of which little information is known.
...
Although deployed rovers have achieved greater coverages, these are subject to slippage-hazardous situations \cite{Gonzalez2017} and face uncrossable obstacles or wind devils. These may damage the rover systems, cause progress delay or determine the end of the mission. An example of it is depicted in figure \ref{fig:highslipevent}.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=0.4\textwidth]{Figures/high_slip_event.png}
  \caption[Wheeled rover under a high-slip event.]{Wheeled rover under a high-slip event \cite{Gonzalez2017}.}
  \label{fig:highslipevent}
\end{figure}

Slippage-related hazards play an important role in preventing a rover to achieve its proposed goals. Even though some slip is desirable to generate traction, excessive slip will result in loss of traction \cite{Wong2001} and possibly entrap the rover. These events are directly linked to the type of wheel-soil interaction at play \cite{Genta2012}. The nature of this interaction takes into account both the type of wheel (rigid or compliable) and its features as well as the type of soil (firm or deformable) and its properties. Apart from it, other variables weighing into slip are the vertical load acting on the soil by the wheel and sinkage, which will cause a decrease on effective thrust and decrease drawbar pull \cite{wong:terramechanics, wong:performance}. Rigid wheels face higher slips on deformable ground, due to high sinkage and shear displacement, while slip is tremendously reduced in the case of compact soils \cite{Green1971}. In Table \ref{tab:roverHazards}, one can find slippage events faced by wheeled, autonomous rovers in extreme environments as well as the teleoperating actions put in place to attempt a reversal from the state the rover was at.
...
As a result of tackling these issues, this thesis will focus on developing a \acrlong{nmpc} method that aims to \textbf{maximize the operational timeline}, by decreasing the time dedicated to traverse the planetary environment to reach locations of scientific interest and increase the available time for scientific operations. Extending the covered surface is also a desirable navigational outcome, which may be eased by providing the robot with a prior knowledge of the scouting area \cite{Coombs2000, Urmson2004}. Therefore, this thesis will focus on providing a navigational control architecture which addresses the issues presented previously, by taking \textbf{safety, speed and covered area as the main metrics of success}.

Apart from that, the control architecture shall capture the dynamics of the mobile robot and be able to cope with rough, uneven terrain. Moreover, the safety of the robot shall be safeguarded not only by the control module, but also by yielding a global path that ensures these metrics of sucess are more easily achieved.
...
Then, the generation of a safe and fast path comes as the next problem to face. From the elevation map of the terrain, a cost map shall be generated, that captures the terrain uneveness, by taking into account the terrain uneveness and the vehicle hull. From there, it may be possible to extract a path from the traversability map.
...
\begin{figure}[!h]
	...
	\begin{tikzpicture}
		...
		\node at (6.0, -1.5) (6) {};
  		...
  		\draw [-latex,thick] (5) --  node [left] {Robot dimensions} (3);
  		\draw [-latex,thick] (6) --  node [right] {Robot hull} (3);
	\end{tikzpicture}
...
In order to implement the project outline, but also to effectively meet the demands of the problem description, the following goals are stated:

\begin{enumerate}
\item \textbf{Dynamics identification}
	\begin{enumerate}
		\item Identification of terrain dynamics regime;
		\item Identification of all external forces acting on a given mobile robot;
		\item Dynamics model validation.
	\end{enumerate}
\item \textbf{Generation of safe, optimal, global path}
	\begin{enumerate}
		\item Generation of a traversability map that expresses the difficulty to traverse each point of the map, with respect to the selected mobile robot hull;
		\item Global path generation that avoids dangerous or untraversable positions and enables a fast traversal of the map.
	\end{enumerate}
\item \textbf{Development of a \acrlong{nmpc} algorithm that copes with extreme terrain}
	\begin{enumerate}
		\item Fast tracking of safe global path by \acrshort{nmpc} algorithm;
		\item Prevent collisions, slippage and hazardous situations;
		\item Verification of controller real-time feasibility;
		\item Algorithm experiment in real-world setting;
	\end{enumerate}
\end{enumerate}
...
Along the work presented in this thesis, the following contributions were delivered:

\begin{enumerate}
	\item Development of a \textbf{worst-case traversability map that considers the terrain roughness and inclination}. On top of it, the traversability map is refined to penalize positions bordering highly untraversable areas. The traversability maps were also generated by taking into account the robot's adaptation capability to the terrain. 
	\item Extensive testing of a real-time \acrlong{nmpc} path-tracking algorithm is also delivered. The algorithm copes well with rough terrain and prevents slippage. Computational feasibility is also guaranteed.
	\item Development of a \textbf{safe, convex real-time path-tracking \acrlong{nmpc} algorithm}. This algorithm is an extension of the second contribution. It sucessfully prevents the mobile robot from entering unsafe areas of the map. It is mainly based on the work of \citet{schoels2020ciao}, but his method was adapted for a wheeled mobile robot traversing irregular, rigid terrain.
\end{enumerate}

%The closest implementation of a similar algorithm that was found in the literature was performed in smooth terrain and did not respect real-time requirements \cite{yu2022nmpc}. Some real-time control applications to rough terrain were found, but these were solely performed along the longitudinal direction \cite{IagnemmaDubowsy2004, rathod2021model}.
...
\begin{itemize}
\item \textbf{Chapter 2}: The literature related to each of the subjects mentioned in section \ref{section:objectives} is laid out. The theoretical background that is behind the development of a global path planner and the \acrshort{nmpc} algorithm is presented.
\item \textbf{Chapter 3}: The dynamics identification and validation of the mobile robot is performed here. Moreover, several assumptions are laid out, in order to simplify the problem's approach.
\item \textbf{Chapter 4}: The implementation of the first contribution of this thesis is detailed. The fast marching method was chosen so that a global optimal potential field properly guides the path generation to the goal without falling into local minimums.
\item \textbf{Chapter 5}: The implementation of the second contribution of this thesis is presented here.
\item \textbf{Chapter 6}: The implementation of the last contribution of this thesis is laid out here.
\item \textbf{Chapter 7}: This chapter reviews the whole work and points to its deficiencies. Some research directions that may be followed on top of this thesis are also proposed.
\end{itemize}