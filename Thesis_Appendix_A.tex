...
\begin{equation}
	\bm{\RotMatrix}_{\BodyFrame}^{\WorldFrame} = \bm{\RotMatrix}_z (\yaw) \bm{\RotMatrix}_y (\pitch) \bm{\RotMatrix}_x (\roll)
\end{equation}
...
\begin{equation}
	\begin{aligned}
	\bm{\RotMatrix}_{\BodyFrame}^{\WorldFrame} (\roll, \pitch, \yaw) &=
	\begin{bmatrix}
		\cos(\yaw) & -\sin(\yaw) & 0 \\
		\sin(\yaw) & \cos(\yaw) & 0 \\
		0 & 0 & 1
...
\label{section:doubleIntegralSimpsonRule}

Simpson's $\frac{1}{3}$ reads as follows:

\begin{equation}
  \int_a^b f(x)\, dx \approx \frac{b - a}{6} \left[f(a) + 4f\left(\frac{a + b}{2}\right) + f(b)\right] = \frac{1}{3} h\left[f(a) + 4f\left(a + h\right) + f(b)\right]
\label{eq:simpsonRule}
\end{equation}

Then, for a double integral, it comes that,
	
\begin{equation}
	\int_c^d \int_{a(x)}^{b(x)} f(x, y)\, dx \, dy \approx
\end{equation}
