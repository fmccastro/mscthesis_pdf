...
This chapter takes on the \acrshort{nmpc} algorithm developed in the previous chapter, and reformulates it in order to prevent collisions and safeguard the vehicle safety. Firstly, the generation of a new and free of collisions reference is outlined in section \ref{section:saferefgen}. Then, this reference is fed to a \acrshort{nmpc} solver that safeguards a minimum clearance of each position horizon node to the nearest obstacle (section \ref{section:realtimesafenmpcroughterrain}). This chapter is mostly based on the work of \citet{schoels2020ciao, schoels2020nmpc}.
...
\tikzstyle{block} = [draw, text centered, minimum height = 2.0em, minimum width = 3.0em, rounded corners, line width = 0.5 mm]
	\begin{tikzpicture}
		...
		-- (3);
  		...
  		\draw [-latex, thick] (fork) -- ++ (0.0, 1.0) -- node [above] {$\mathbf{p}^*$} ++(-4.0, 0.0) -- (6);
...
The set of equations \eqref{eq:lagrangemayersafeocp} and \eqref{eq:safeonestageocp} define the cost terms and the linear and non-linear constraints of this new safe, convex \acrshort{nmpc}, respectively. Equation \eqref{subeq:ciaocirclesarea} is the new appended non-linear constraint. This constraint sets the admissable area for each position of the optimal horizon. $\mathbf{c}_k^*,\,d_\ObstacleSpace(\mathbf{c}_k^*)$ and $d_s$ are parameters of the problem that come from the results of algorithm \nameref{alg:computefreespace}.

Secondly, another major novelty is the introduction of slacks. The need to relax constraint \eqref{subeq:ciaocirclesarea} comes from the observation that the \acrshort{nmpc} sometimes becomes infeasible if $\mathbf{p}^*$ is pushed to the border of $\SafeSpace_{\mathbf{c}^*}$. Slacks are non-negative (\ref{subeq:slackbounds}) and are also weighted in the cost function terms \eqref{subeq:onestagelagrangesafeocp} and \eqref{subeq:onestagemayersafeocp}. $Z_k \gg 1$ so that $s_k$ is only active when it is of the utmost importance to prevent infeasibility \cite{schoels2020nmpc}.
\begin{subequations}
\begin{gather}
\lagrangeTerm{\mathbf{\x},\,\mathbf{u},\,\mathbf{r}_{def},\,s}_k = \Vert\x - r_{\x,\,def},\,\y - r_{\y,\,def},\,e_{\yaw},\,v_x,\,\omega_z,\,\delta \Traction_l,\,\delta \Traction_r\Vert_{Q,\,k}^2 \; + \Vert u_l,\,u_r \Vert_{R,\,k}^2 + \frac{1}{2}\,s_k^2\,Z_k+ s_k\,z_k\label{subeq:onestagelagrangesafeocp}\\
\mayerTerm{\mathbf{\x},\,\mathbf{r}_{def},\,s}_k = \Vert\x - r_{\x,\,def},\,\y - r_{\y,\,def},\,e_{\yaw},\,v_x,\,\omega_z,\,\delta \Traction_l,\,\delta \Traction_r\Vert_{Q,\,k}^2 + \frac{1}{2}\,s_k^2\,Z_k+ s_k\,z_k\label{subeq:onestagemayersafeocp}
\end{gather}
...
The list of states and controls remains unchanged. The slacks horizon, $\mathbf{s} = \left[s_0,\,s_1,\ldots,s_N\right]$, also becomes an optimization variable of the problem. Moreover, the set of parameters becomes $\mathbf{p} = \left[\FrictionCoefficient,\,m,\,g,\,\InertiaTensor_{zz},\,d_s,\,\mathbf{c}^*,\,d_\ObstacleSpace(\mathbf{c}^*),\,\mathbf{r}_{def}\right]$, where $\mathbf{c}^* = \left[\mathbf{c}_0^*,\,\mathbf{c}_1^*,\,\ldots,\,\mathbf{c}_N^*\right]$ encapsulates the list of the safe balls centers. Finally, $\mathbf{r}_{def} = \left[\x_0,\,\y_0,\,\yaw_0,\,\x_1,\,\y_1,\,\yaw_1,\,\ldots,\,\x_N,\,\y_N,\,\yaw_N\right]_{def}$. This \acrshort{nmpc} preserves convexity as cost terms are all positive semi-definite and constraints are circles or linear intervals.

\subsection{Slacks behaviour}

From now on, the results of the application of this safe, convex \acrshort{nmpc} are presented and discussed. Table \ref{tab:SafeRoughPathTrackingSpecs} denote the cost terms of the solvers that are tested in this subsection. Solver \ti{Low slacks} gives much less room for the slacks to increase than solver \ti{High slacks}. From this subsection to the end of the chapter, $d_s = \underline{d}$. The rest of the parameters are equal to the ones mentioned in figure \ref{fig:potentialfieldresults}. The heightmap and virtual speed control module are the same of subsection \ref{subsection:traversalRoughTerrain}.

\begin{table}[!htb]
  \caption[Safe, convex \acrshort{nmpc} cost terms and parameters.]{Safe, convex \acrshort{nmpc} cost terms and parameters ($N = 40,\; T_s = 40\,ms$).}
  \label{tab:SafeRoughPathTrackingSpecs}
  \renewcommand{\arraystretch}{1.2}
  \centering
  \begin{tabular}[]{lcc}
	\toprule
	Solver & $(1)$ Low slacks & $(2)$ High slacks \\
	\midrule
...
Figure \ref{fig:pathtrackingsafeocp} assembles the path tracking results of solver \ti{Low slacks} at different initial conditions. Reference tracking and obstacle avoidance is successfully achieved. The mobile robot also does not enter $\overline{\SafeSpace}$, but moves alongside the border at most. On top of the bottom left obstacle, the mobile robot turns around because the target is so far ahead that the potential field guides the deformed reference to the right, instead of circling the obstacle.

On the other hand, figure \ref{fig:reftoursolver2} presents path tracking results with solver \ti{High slacks}. In this case, the mobile robot enters $\overline{\SafeSpace}$. This happens because the weight that penalize the slacks is much smaller, which allows the slacks to be greater. Due to that, the solver expands the admissable area in such a way that there is a big intersection between constraint \eqref{subeq:ciaocirclesarea} and $\overline{\SafeSpace}$. It is clear that this solver configuration is undesirable as the mobile robot is placed in risky situations. Then, the purpose of slacks is to provide flexbility to the \acrshort{nmpc} solver, in order to allow the mobile robot to briefly enter $\overline{\SafeSpace}$, but also to avoid the solver failure. The same problem was run without slacks and infeasibility typically happened when the circles are very small and the vehicle is at the border of the \acrshort{ciao} circles. 

\begin{figure}[!htb]
  \begin{subfigmatrix}{2}
    \subfigure[Reference tour.]{\includegraphics[width=0.49\linewidth]{Figures/Obstacle avoidance/Safe NMPC/tour_part1_solver_1.pdf}}
    \subfigure[Path tracking from different initial poses.]{\includegraphics[width=0.49\linewidth]{Figures/Obstacle avoidance/Safe NMPC/Several_paths.pdf}}
  \end{subfigmatrix}
  \caption{Path tracking with safe one-stage real-time \acrshort{nmpc} with solver \ti{Low slacks}.}
...
\begin{minipage}{.49\textwidth}
	\centering
  	...
  	\captionof{figure}{Traction forces within coulomb friction cone (solver \ti{Low slacks}).}
...
\begin{minipage}{.49\textwidth}
	\centering  
  	...
  	\captionof{figure}{Reference tour with solver \ti{High slacks}.}
...
Along the whole reference tour, the minimum recorded clearance was $0.287\,m$ (solver \ti{Low slacks}). In the case of solver \ti{High slacks}, the minimum recorded clearance was $0.12\,m$. This shows that solver \ti{Low slacks} safeguards the mobile robot's safety, whereas solver \ti{High slacks} compromises it.
...
\begin{figure}[!htb]
  ...
  \end{subfigmatrix}
  \caption{Reference tour performance plus velocities and force rates curves with solver \ti{Low slacks}.}
...
A video\footnote{Results of the safe one-stage \acrshort{nmpc} real-time with solver $(1)$: \texttt{\url{https://youtu.be/xHckwLaS_dQ}} (accessed on 14 of February 2026)} was prepared to showcase the results of this chapter. The results rendered in that video were generated with solver \ti{Low slacks}, under the same parameters that generated the results of this subsection.
