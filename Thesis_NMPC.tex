...
The simulation results of the planar \acrshort{nmpc} with respect to the control horizon are presented in figure \ref{fig:PathTrackCompHorizonVar} and table \ref{tab:cumulativeControlEffortControlHorizon}. The parameters of the solver that are kept constant on every simulation are stated in table \ref{tab:controlHorizonSpecs}. The order of $Q$ and $R$ diagonal elements is aligned with the order of the cost terms presented in equations \eqref{eq:reflagrange} and \eqref{subeq:onestagemayer}.
...
Likewise, the constant parameters are presented in table \ref{tab:samplingTimeSpecs}. The respective results are presented in figure \ref{fig:PathTrackCompSamplingTimeVar} and table \ref{tab:cumulativeControlEffortSamplingTime}.
...
\begin{table}[!htb]
  \caption[Constant solver parameters when cost function weights change.]{Solver parameters ($N = 50,\, T_s = 0.01 \, ms$).}
...
\begin{figure}[!htb]
  \begin{subfigmatrix}{2}
    \subfigure[Path tracking over hill with different conditions.]{\includegraphics[width=0.49\linewidth]{Figures/Real time one stage OCP on rough terrain/Hill/Path tracking comparison among hills.pdf}}
...
\begin{table}[!htb]
  \caption[Solver parameters for simulation over irregular terrain.]{Path tracking solver parameters ($N = 50,\; T_s = 20\,ms$).}
...
\begin{table}[!htb]
  \caption[Solver performance over irregular, rigid terrain.]{Solvers performance on rough, uneven terrain. On every solver, one timeout was recorded.}
