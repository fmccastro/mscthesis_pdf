...
The mobile robot position and orientation with respect to frame $\WorldFrame$ is described as $\mathbf{\Position} = [x\;  y\; z]^T$ and $\bm{\Orientation} = [\roll \; \pitch \; \yaw]^T$, respectively. On the other hand, the mobile robot linear and angular velocities with respect to frame $\BodyFrame$ stands as $\mathbf{\LinVelocity} = [ \vx \; \vy \; \vz ]^T$ and $\bm{\AngVelocity} = [\wx \; \wy \; \wz]^T$, respectively. Then, it is possible to map the mobile robot $\Robot$ linear velocity between frames $\BodyFrame$ and $\WorldFrame$ by taking advantage of rotation matrix \ref{eq:rotationMatrixW2B}, but also to map euler angle rates in $\WorldFrame$ to angular velocities in $\BodyFrame$ with rotation matrix \ref{eq:orientationQuatRotationMatrix}.
...
In this thesis, it is important to retrieve each wheel's contact point with the terrain it is touching on, in order to enable the transfer of forces applied at $\contactPoint_i$ to frame $\BodyFrame$ and to describe the velocity of each wheel with respect to a frame $\WheelFrame_i$.
...
At every wheel $i$, a frame $\WheelFrame_i$ is attached to the wheel center and follows up the terrain profile slope longitudinally. In accordance with assumption \ref{assumption:singlePointContact}, roll effect on the determination of $\contactPoint_i$ is neglected. The transformation from $\WheelFrame_i$ to $\BodyFrame$ is based on a rotation matrix expressed in equation \ref{eq:rotationMatrixB2T}. Frame $\WheelFrame^i$ cannot yaw with respect to $\BodyFrame$ because the wheels do not steer ($\yaw_i = 0$) and the lateral component, $\WheelFrame_{y,\,i}$, matches with the wheel axis of rotation ($\roll_i = 0$).

\begin{equation}
\mathbf{\RotMatrix}_{\WheelFrame^i}^{\BodyFrame} =
\begin{bmatrix}
 		\cos(\pitch_i) & 0 & \sin(\pitch_i) \\
 		0 & 1 & 0 \\
...
, where $\pitch_i$ denotes the pitch component of the contact angle. Finally, it is possible to express $\mathbf{\LinVelocity}_i$ with respect to frame $\WheelFrame^i$ (equation \ref{eq:wheelVelT}). This reasoning is depicted in figure \ref{fig:wheelKinematics}. 
...
\subsubsection*{Contact angle determination}

Contact angle determination is of the utmost importance in case the mobile robot is crossing irregular terrains, because the traction forces are not aligned with $\BodyFrame_x$. A method to compute $\pitch_i$ is extracted from \cite{IagnemmaDubowsy2004}. In this thesis, only the equations to determine $\pitch_i$ are presented. Further explanations can be found in \cite{IagnemmaDubowsy2004}, although it is worth mentioning that special cases are not included here.
...
\begin{equation}
	\delta_i = \WheelRadius \AngVelocity_i - \left( \vx_i \right)_{\WheelFrame_i}
...
The presented wheeled mobile robot model contains four controllable joints at each wheel \acrshort{com}, which are located at each wheel's center. The mobile robot may roll, pitch or fall vertically with respect to frame $\BodyFrame$, because it will be moving through irregular terrain, which changes the balance of forces the mobile robot is subject to. Moreover, external forces (e.g. friction contact, wheel-terrain traction) and gravity are also taken into account in this general model. Typically, wind force is also an external force which acts on the mobile robot body at outdoor environments, but its effect was not introduced in this model.
...
Every wheel is commanded by a torque, $\bm{\Torque}_i$, which  is expressed with respect to $\BodyFrame$ and it only holds a component in $\BodyFrame_y$. That torque yields a traction force $\mathbf{\Traction}_i$ which is expressed with respect to the wheel frame $\WheelFrame_i$ (figure \ref{fig:wheelForcesMoments}). The traction force $\mathbf{\Traction}_i$ only holds a single longitudinal component within $\WheelFrame^i$. $\mathbf{\Traction}_i$ is the result of the action of the soil on the wheel at $\contactPoint_i$. Therefore, the force produced by $\bm{\Torque}_i$ on the soil opposes $\mathbf{\Traction}_i$. The wheel frame $\WheelFrame_i$ is centered at the wheel center and follows up the wheel velocity vector, which may not match frame $\BodyFrame_i$. Finally, it comes that,

\begin{equation}
	\mathbf{\Traction}_i = \begin{cases}
		\frac{ \bm{\Torque}_i }{\WheelRadius}, \; \left| \frac{ \bm{\Torque}_i }{\WheelRadius} \right| - \FrictionCoefficient \left( \Force_{\NormalForceSubscript} \right)_i < 0 \\
		\FrictionCoefficient \left( \Force_{\NormalForceSubscript} \right)_i \text{sign} \left( \bm{\Torque}_i \right), \, \left| \frac{ \bm{\Torque}_i }{\WheelRadius} \right| - \FrictionCoefficient \left( \Force_{\NormalForceSubscript} \right)_i \geq 0  
	\end{cases}	
...
, where $\mathbf{\contactPoint}_i$ is a vector connecting the mobile robot \acrshort{com} to the contact point $\contactPoint_i$, with respect to $\BodyFrame$. Due to assumption \ref{eq:momentsNeglected}, $\mathbf{\Moment}_{\mathbf{\Traction}}$ does not include $\bm{\Torque}_i$.
...
\begin{equation}
\left( \ForceSubscript{\FrictionSubscript} \right)_i \leq \FrictionCoefficient \left( \ForceSubscript{\NormalForceSubscript} \right)_i
\label{eq:simpleFriction}
\end{equation}

If a wheel $i$ sticks to the contact point $\contactPoint_i$, it means equation \ref{eq:simpleFriction} is inactive. On the other hand, if a wheel is sliding, equation \ref{eq:simpleFriction} is active and $\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i$ has an opposite direction to $\left( \mathbf{\LinVelocity}_i \right)_{\WheelFrame^i}$.
...
\begin{equation}
	\begin{cases}
		...
		\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i = -\FrictionCoefficient \left( \ForceSubscript{\NormalForceSubscript} \right)_i \cdot \left( \frac{\mathbf{\LinVelocity}_i}{ \| \mathbf{\LinVelocity}_i \| } \right)_{\WheelFrame^i}, \; \mathbf{\LinVelocity}_i \neq \mathbf{0} \\ \vspace{2mm}
		\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i = -\FrictionCoefficient \left( \ForceSubscript{\NormalForceSubscript} \right)_i \cdot \left( \frac{\mathbf{\LinAccel}_i}{ \| \mathbf{\LinAccel}_i \| } \right)_{\WheelFrame^i}, \; \mathbf{\LinVelocity}_i = \mathbf{0} \wedge \mathbf{\LinAccel}_i \neq \mathbf{0}
	\end{cases}
	\label{eq:friction}
\end{equation}
...
Motivated from equations \ref{eq:sumtractions}, \ref{eq:sumMomentsTraction}, 
\ref{eq:frictionMomentForce} and \ref{eq:sumGravityReaction}, $\mathbf{f}$ and $\mathbf{\Moment}$ are represented with respect to the actuation wheel torques, $\bm{\Torque}_i$.
...
\mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_1 \right)  \mathbf{A}_1 & \mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_2 \right)  \mathbf{A}_2 & \mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_3 \right) \mathbf{A}_3 & \mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_4 \right) \mathbf{A}_4
\end{bmatrix} \begin{bmatrix}
\bm{\Torque}_1 \\
\bm{\Torque}_2 \\
\bm{\Torque}_3 \\
\bm{\Torque}_4 
\end{bmatrix} + \mathbf{1}_{6 \times 6} \begin{bmatrix}
\mathbf{\Force_{\FrictionSubscript}} & \mathbf{\Force_{\gravity}} & \mathbf{\Force_{\NormalForceSubscript}} \\
\mathbf{0} & \mathbf{0} & \mathbf{0}
\end{bmatrix} \mathbf{1}_{3 \times 3}
\label{eq:forcesMomentsMatrix}
\end{equation}

, where $\mathbf{A}_i = \mathbf{\RotMatrix}_{\WheelFrame_i}^{\BodyFrame} \left( \mathbf{\SkewSymmetricMatrix}^T \left( \mathbf{\wheelToContact}_i \right) \mathbf{\RotMatrix}_{\WheelFrame_i}^{\BodyFrame} \right)^{\dagger}$.
