...
The problem laid out in this thesis is to yield a Nonlinear Model Predictive Control (\acrshort{nmpc}) \cite{Rawlings2013, grunenmpc} algorithm $\mathbb{P}$, which models the kinematics and dynamics of a rigid wheeled, autonomous robot $\mathcal{R}$ as well as its interactions with the terrain during the traversal of a semi-unknown, off-the-road, uneven terrain map $\mathcal{M} \subset \mathbb{R}^3$, resembling a planetary environment. It is required that the robot avoids untraversable areas or obstacles $\mathcal{\ObstacleSpace} \subset \mathcal{\Map}$, and follows a time optimal traversable pathway to reach a goal point $\mathnormal{G} \in \mathcal{M}$, starting from a given initial admissable pose $\mathcal{R}_p \in \Map \setminus \mathcal{\ObstacleSpace}$. Moreover, the robot $\mathcal{R}$ shall make a fast, safe and slippage-free traverse, as well as minimizing the energy consumed by the robot battery.  The algorithm $\mathbb{P}$ must be computationally optimal and update the system actuation in a timely fashion.
...
Due to its compactness and simplicity, \acrlong{dem} are chosen to describe the elevation $\Elevation$ at each location $(x_i, y_j)$. Then, from the compacted data, a cost map approach is followed in order to yield a traversability map  of the environment, by assigining a traversability cost $\TraversabilityCost$ to each grid point $(x_i, y_j)$. This choice stems from the fact that this is a popular trend in the planetary robotics literature, but also because it is easily combined with a minimum-cost planner algorithm, in order to output a global planner. Morever, these frames are considered to share the same frame $\WorldFrame$, because both map frames match at the origin. An algorithm to address this plan is presented in section \ref{section:costMap}. It is built on top of the works of \citet{Amorim2014} and \citet{Ishigami2007PathPlanning} and provides an assignment of the traversability cost to each position of the map, based on the map's terrain roughness and inclination, followed by the computation of a time-optimal path that takes into account the latter information.
...
The mobile robot position and orientation with respect to frame $\WorldFrame$ is described as $\mathbf{\Position} = [x\;  y\; z]^T$ and $\bm{\Orientation} = [\roll \; \pitch \; \yaw]^T$, respectively. On the other hand, the mobile robot linear and angular velocities with respect to frame $\BodyFrame$ stands as $\mathbf{\LinVelocity} = [ \vx \; \vy \; \vz ]^T$ and $\bm{\AngVelocity} = [\wx \; \wy \; \wz]^T$, respectively. Then, it is possible to map the mobile robot $\Robot$ linear velocity between frames $\BodyFrame$ and $\WorldFrame$ by taking advantage of rotation matrix \ref{eq:rotationMatrixW2B}, but also to map euler angle rates in $\WorldFrame$ to angular velocities in $\BodyFrame$ with rotation matrix \ref{eq:rotationMatrixW2BRotationalInverse}.
...
\subsection{Wheel-terrain contact}
\label{subsection:contactAngles}

In this thesis, it is important to retrieve each wheel's contact point with the terrain it is touching on, in order to enable the transfer of forces applied at $\contactPoint_i$ to frame $\BodyFrame$ and to describe the velocity of each wheel with respect to a frame $\WheelFrame^i$.

\begin{assumption}
	Every wheel $i$ is touching the terrain at a point $\contactPoint_i$.
...
At every wheel $i$, a frame $\WheelFrame_i$ is attached to the wheel center and follows up the terrain profile slope longitudinally. In accordance with assumption \ref{assumption:singlePointContact}, roll effect on the determination of $\contactPoint_i$ is neglected. The transformation from $\WheelFrame^i$ to $\BodyFrame$ is based on a rotation matrix expressed in equation \ref{eq:rotationMatrixB2T}.

\begin{equation}
\mathbf{\RotMatrix}_{\WheelFrame^i}^{\BodyFrame} =
\begin{bmatrix}
 		\cos(\yaw_i)\cdot \cos(\pitch_i) & -\sin(\yaw_i) & \cos(\yaw_i)\cdot \sin(\pitch_i) \\
 		\sin(\yaw_i) \cos(\pitch_i) & \cos(\yaw_i) & \sin(\yaw_i) \sin(\pitch_i) \\
 		-\sin(\pitch_i) & 0 & \cos(\pitch_i) \\
	\end{bmatrix}
	\label{eq:rotationMatrixB2T}
\end{equation}

, where $\pitch_i$ denotes the pitch component of the contact angle and $\yaw_i$ stands for the steering component. Finally, it is possible to express $\mathbf{\LinVelocity}_i$ with respect to frame $\WheelFrame^i$ (equation \ref{eq:wheelVelT}). This reasoning is depicted in figure \ref{fig:wheelKinematics}, where the neglection of $\roll_i$ is retracted for the sake of completeness.
...
\begin{assumption}
	The roll component of $\WheelFrame^i$ with respect to $\BodyFrame$ is neglected because the traction force and wheel velocity components on $\WheelFrame_y^i$ are very small, apart from high lateral skid situations.
\end{assumption}

\subsubsection*{Contact angle and contact points determination}

Contact angle determination is of the utmost importance in case the mobile robot is crossing irregular terrains, because the traction forces are not aligned with $\BodyFrame_x$. A method to compute $\pitch_i$ can be found in \cite{IagnemmaDubowsy2004}. Further explanations can be found in \cite{IagnemmaDubowsy2004}, although it is worth mentioning that special cases are not included here.

In this thesis, the contact angle is determined by extracting the contact normal at contact point $c_i$ directly from the \textsc{Gazebo} simulation, as well as every $c_i$. In \textsc{Gazebo}, the contact points are expressed with respect to $\WorldFrame$, so equation may be employed to get $c_i$ with respect to $\BodyFrame$.

\begin{equation}
	\left(\mathbf{\contactPoint}_i\right)_{\WorldFrame} = \mathbf{\Position} + \mathbf{\RotMatrix}_{\BodyFrame}^{\WorldFrame} \mathbf{\contactPoint}_i
\label{eq:contactPointWorld}
\end{equation}

%In the end, I have to decide if I place the below commented block on annex
...
%
%\textcolor{red}{Simulate robot first without contact angle (assume plane of contacts is parallel to the same plane of the robot; then, include this estimation into the simulation; at rest (for maps generation), use gradients}
%
%\textcolor{red}{Don't forget to include the pure rolling condition; it is violated when there is slip}

\subsection{Slippage}
\label{subsection:slippage}
...
\begin{equation}
  	\begin{aligned}
      		\SlipRatio_i = \begin{cases}
						1 - \frac{\vx_i}{\WheelRadius \; \AngVelocity_i}, \quad \text{if} \quad |\WheelRadius \; \AngVelocity_i| \geq | \vx_i| \; \text{(slip)}\\
						\frac{\WheelRadius \; \AngVelocity_i}{\vx_i} - 1, \quad \text{if} \quad |\WheelRadius \; \AngVelocity_i| < | \vx_i| \; \text{(skid)}
							\end{cases}
    	\end{aligned} \qquad
    	\begin{aligned}
...
, where $\WheelRadius$ denotes the wheel radius. Beware that every linear velocity is expressed with respect to $\WheelFrame^i$ in \ref{eq:slip} and \ref{eq:relativeVelocity}. Physical parameters related to every wheel are equal. If $0 \leq \SlipRatio_i \leq 1$, it is said wheel $i$ is slipping. If $-1 \leq \SlipRatio_i < 0$, it is said that wheel $i$ is skidding or braking. However, if the signs of $\vx_i$ and $\AngVelocity_i$ are opposite to each other, it means $1 < |\SlipRatio_i| \leq 2$. Although these impractical slips are possible \cite{Peiret2021}, no terramechanics analytical models are presented, because no literature was found with research on the topic. Regarding the slip angle $\SlipAngle$, this quantity is as high as the lateral skid, $\vy$, is with respect to the longitudinal velocity, $\vx$.
...
\begin{equation}
	\delta_i = \WheelRadius \AngVelocity_i - \vx_i			   
...
\begin{lemma}
Pure rolling condition is respected in case equation \ref{eq:relativeVelocity} equals zero.
\label{lemma:pureRollingCondition}
\end{lemma}
...
\begin{figure}[!h]
	...
	\begin{tikzpicture}
		...
		\node at (5.5, -1.5) (9) {};
  		...
  		\draw [-latex,thick] (4) --  node [above] {$\mathbf{\Position}, \; \bm{\Orientation}$} (5);
...
Each wheel joint can only be actuated around the wheel horizontal axis, which is parallel to $\WheelFrame_y^i$. Then, it also holds a frame $\WheelFrame^i$ which follows up the terrain slope (section \ref{subsection:contactAngles}). Apart from it, each wheel is subject to forces and moments coming from its interaction with the terrain or due to gravity (figure \ref{fig:wheelForcesMoments}). Finally, these forces and moments shall be expressed with respect to frame $\BodyFrame$ and summed up in order to feed the dynamics relationship of the mobile robot (equation \ref{eq:dynamics}).
...
Every wheel is commanded by a torque, $\bm{\Torque}_i$. That torque yields a traction force $\mathbf{\Traction}_i$ which is expressed with respect to the wheel frame $\WheelFrame^i$ (figure \ref{fig:wheelForcesMoments}). The traction force $\mathbf{\Traction}_i$ only holds a single longitudinal component within $\WheelFrame^i$. $\mathbf{\Traction}_i$ is the result of the action of the soil on the wheel at $\contactPoint_i$. Therefore, the force produced by $\bm{\Torque}_i$ on the soil opposes $\mathbf{\Traction}_i$. Finally, it comes that,

\begin{equation}
	\mathbf{\Traction}_i = \begin{cases}
		\frac{ \Torque_i }{\WheelRadius}, \; \left| \frac{ \Torque_i }{\WheelRadius} \right| - \FrictionCoefficient \left( \Force_{\NormalForceSubscript} \right)_i < 0 \\
		\FrictionCoefficient \left( \Force_{\NormalForceSubscript} \right)_i \text{sign} \left( \Torque_i \right), \, \left| \frac{ \Torque_i }{\WheelRadius} \right| - \FrictionCoefficient \left( \Force_{\NormalForceSubscript} \right)_i \geq 0  
	\end{cases}	
...
%\begin{assumption}
%Moments produced around $\BodyFrame_x$ and $\BodyFrame_y$ are neglected because the terrain counter-acts its effects.
%\label{eq:momentsNeglected}
%\end{assumption}
...
\begin{equation}
	\mathbf{\Moment}_{\mathbf{\Traction}} = \sum_{i = 1}^{4} \mathbf{\contactPoint}_i \times \left( \mathbf{\RotMatrix}_{\WheelFrame^i}^{\BodyFrame} \mathbf{\Traction}_i \right)
...
%Finally, if the sum of moments and forces is presented with respect to the actuation $\bm{\Torque}_i$, it comes that

%\begin{equation}
%\mathbf{\Force}_{\mathbf{\Traction}} = \sum_{i=1}^{4} \mathbf{\RotMatrix}_{\WheelFrame_i}^{\BodyFrame} \left( \mathbf{\SkewSymmetricMatrix}^T \left( \mathbf{\wheelToContact}_i \right) \mathbf{\RotMatrix}_{\WheelFrame_i}^{\BodyFrame} \right)^{\dagger} \bm{\Torque}_i
%\label{eq:sumtractions}
%\end{equation}
%
%\begin{equation}
%\mathbf{\Moment}_{\mathbf{\Traction}} = \sum_{i=1}^{4} \mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_i \right) \mathbf{\RotMatrix}_{\WheelFrame_i}^{\BodyFrame} \left( \mathbf{\SkewSymmetricMatrix}^T \left( \mathbf{\wheelToContact}_i \right) \mathbf{\RotMatrix}_{\WheelFrame_i}^{\BodyFrame} \right)^{\dagger} \bm{\Torque}_i
%\label{eq:sumMomentsTraction}
%\end{equation}

%\subsection{Wheel spin rate}
%\label{subsection:wheelSpinRate}
%
%Each wheel is considered to spin only if a torque $\bm{\Torque}_i$ is fed to the wheel along its lateral axis.
%
%\begin{equation}	
%	\mathbf{\InertiaTensor}_{\text{wheel}} \, \bm{\dot{\AngVelocity}}_i = \bm{\Torque}_i
%	\label{eq:wheelSpinRate}
%\end{equation}
%
%Equation \ref{eq:wheelSpinRate} is expressed with respect to $\BodyFrame$.
%
%\begin{assumption}
%	The wheel is considered to be locked, so its motion is only due to actuation control.
%	\label{assumption:wheellock}
%\end{assumption}
...
There are four possible cases of wheel-terrain interaction (figure \ref{fig:contactModels}): rigid wheel on rigid terrain, rigid wheel on deformable terrain, deformable wheel on rigid terrain and a deformable wheel on deformable terrain. In \cite{Genta2012, Yoshida2016}, one can find analytical models for all these possibilities.

\begin{figure}[!htb]
  \centering
  \begin{subfigmatrix}{4}
  	\subfigure[Rigid wheel contacting with rigid terrain]{\includegraphics[width=0.15\linewidth]{Figures/rigid_rigid.png}}
    \subfigure[Rigid wheel contacting with deformable terrain]{\includegraphics[width=0.15\linewidth]{Figures/rigid_deformable.png}}
    \subfigure[Deformable wheel contacting with rigid terrain]{\includegraphics[width=0.15\linewidth]{Figures/deformable_rigid.png}}
    \subfigure[Deformable wheel contacting with deformable terrain]{\includegraphics[width=0.15\linewidth]{Figures/deformable_deformable.png}}
  \end{subfigmatrix}
  \caption{Wheel-terrain contact models \cite{Yoshida2016}}
  \label{fig:contactModels}
\end{figure}

In this thesis, only rigid wheels are considered. Even if a wheel presents some degree of deformability (i.e. pneumatic tire), it can be modeled as a rigid wheel if the average tire ground pressure is higher than the critical ground pressure, $p_{gcr}$ \cite{Yoshida2016}.
...
From assumption \ref{eq:sameFrictionCoefficients}, and if one states $\left( \ForceSubscript{\FrictionSubscript} \right)_i^2 = \left( \ForceSubscriptTwo{\FrictionSubscript}{x} \right)_i^2 + \left( \ForceSubscriptTwo{\FrictionSubscript}{y}\right)_i^2$, it comes that,

\begin{equation}
\left( \ForceSubscript{\FrictionSubscript} \right)_i^2 \leq \left( \FrictionCoefficient \left( \ForceSubscript{\NormalForceSubscript} \right)_i \right)^2
...
As a matter of fact, $\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i = \mathbf{t}_i$. In fact, equation \ref{eq:actuatedForce} also includes the Coulomb friction model. Therefore, the traction force magnitude is limited by the Coulomb friction model.

\subsubsection*{Terrain reaction forces}
\label{subsubsection:coulombFrictionModel}

Every time a contact between a wheel and terrain takes place, a reaction force $\mathbf{\ForceSubscript{\NormalForceSubscript}}$ shows up. The magnitude of this reaction force determines the amount of admissable traction force, before soil failure occurs.
...
\begin{minipage}{0.45\linewidth}
	\begin{equation}
	\mathbf{\ForceSubscript{\NormalForceSubscript}} = \sum_{i = 1}^{4} \mathbf{\RotMatrix}_{\WheelFrame^i}^{\BodyFrame} \left( \mathbf{\ForceSubscript{\NormalForceSubscript}} \right)_i
	\label{eq:totalReactionForce}
\end{equation}
\end{minipage}
\begin{minipage}{0.45\linewidth}
	\begin{equation}
	\mathbf{\Moment_\NormalForceSubscript} = \sum_{i = 1}^{4} \mathbf{\contactPoint}_i \times \left( \mathbf{\RotMatrix}_{\WheelFrame^i}^{\BodyFrame} \, \mathbf{\ForceSubscript{\NormalForceSubscript}}_i \right)
	\label{eq:totalReactionMoments}
\end{equation}
\end{minipage}

%In order to introduce equation \ref{eq:friction} into an \acrshort{ode}, its discontinuities need to be smoothed. From this observation, $\mathbf{\LinVelocity}_i$ is kept as a \ti{breaker}, and the role of $\mathbf{\LinAccel}_i$ in the Coulomb friction model is discarded for the sake of simplicity, thus eliminating the last line of equation \ref{eq:friction}. Motivated from this statement, equation \ref{eq:friction} is transformed into,
%
%\begin{equation}
%\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i = - \left( \mathbf{\ForceSubscript{\Traction}} + \mathbf{\ForceSubscript{\gravity}} + \mathbf{\ForceSubscript{\NormalForceSubscript}} \right)_i \cdot \left(1 - \sigma \left( \Vert \mathbf{\LinVelocity}_i \Vert \right) \right) - \FrictionCoefficient \left( \ForceSubscript{\NormalForceSubscript} \right)_i \cdot \frac{\mathbf{\LinVelocity}_i}{ \| \mathbf{\LinVelocity}_i \| + \epsilon } \cdot \sigma \left( \Vert \mathbf{\LinVelocity}_i \Vert \right)
%\label{eq:smoothFriction}
%\end{equation}

%, where $\sigma \left( \cdot \right)$ denotes the sigmoid function, with $k = 100$, and $\epsilon = 1\mathrm{e}^{-6}$, denotes a very small value that keeps $\frac{\mathbf{\LinVelocity}_i}{ \| \mathbf{\LinVelocity}_i \|}$ differentiable at $\| \mathbf{\LinVelocity}_i \| = 0$.

%Then, the sum of friction forces $\mathbf{\ForceSubscript{\FrictionSubscript}}$ acting on the mobile robot is given by,
%
%\begin{equation}
%	\mathbf{\ForceSubscript{\FrictionSubscript}} = \sum_{i=1}^{4} \mathbf{\RotMatrix}_{\WheelFrame^i}^{\BodyFrame} \left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i
%	\label{eq:frictionMomentForce}
%\end{equation}
%
%Finally, the determination of $\left( \mathbf{\ForceSubscript{\NormalForceSubscript}} \right)_i$ is made by computing the portion of the mobile robot weight every wheel $i$ supports, which is presented below.
%
%\subsubsection*{Effects of gravity}
%\label{subsubsection:effectsGravity}
%
%Gravity pushes the mobile robot into the terrain. As a response, ground reaction forces $\left( \mathbf{\ForceSubscript{\NormalForceSubscript}} \right)_i$ sustain the mobile robot. At each wheel $i$, the effect of gravity acting at $\contactPoint_i$ is denoted by $\left( \mathbf{\ForceSubscript{\gravity}} \right)_i$, which is expressed with respect to $\WheelFrame_i$. The vertical component of $\left( \mathbf{\ForceSubscript{\gravity}} \right)_i$ equals $\left( \mathbf{\ForceSubscript{\NormalForceSubscript}} \right)_i$ and are orthogonal to $\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i$  and $\mathbf{\Traction}_i$. The sum of $\left( \mathbf{\ForceSubscript{\gravity}} \right)_i$ for every wheel equals the mobile robot weight \freefootnote{$\mathbf{\Moment}_{\left( \mathbf{\ForceSubscript{\NormalForceSubscript}} \right)} = \mathbf{\Moment}_{\left( \mathbf{\ForceSubscript{\gravity}} \right)} = \mathbf{\Moment}_{\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)} = \mathbf{0}$}.
%
%\begin{equation}
...
%\end{equation}
%
%, where $\mathbf{\gravity}$ denotes the gravity vector with respect to $\WorldFrame$. The sum of the effects of gravity and ground reaction forces with respect to $\BodyFrame$ are stated by,
%
%\begin{equation}
...
\label{section:modelVerification}

The applicability of equations \ref{eq:fullStateSpace} to a rigid wheeled mobile robot may be tested by comparing the simulated states $\mathbf{x}_{sim}$ collected from \textsc{Gazebo} and the states $\mathbf{x}$ generated from the propagation of the differential equation \ref{eq:fullStateSpace}.

Although the mobile robot is subject to its weight, that enforces a continuous contact with the terrain, the contact forces generated by \textsc{Gazebo} may include unmodeled damping or ground penetration that \ref{eq:fullStateSpace} is not able to model. These unmodeled features seem to create a mismatch between $\mathbf{x}_{sim}$ and $\mathbf{x}$. So, in order to validate model \ref{eq:fullStateSpace}, the wheeled mobile robot is placed on a free-floating environment, where gravity is switched off and no contacts with the terrain take place. Within this setup, some $\mathbf{\Force}$ and $\mathbf{\Moment}$ are fed to the plant and the simulated states $\mathbf{x}_{sim}$ are captured while the simulation is running. This setup is also a valid one for validation, because one may think of the wheel-terrain contacts forces, the vehicle weight and additional moments, as external forces and moments that are acting on a rigid body when it floats in space without contacts.

Finally, the propagated states $\mathbf{x}$ are generated by replacing the respective $\mathbf{\Force}$ and $\mathbf{\Moment}$ to \ref{eq:fullStateSpace} on every iteration of the integrator. This integrator is formulated with the \texttt{AcadosSimSolver()} class from \textsc{acados}. The explicit four stage Runge-Kutta (see \ref{section:rk4}) is the numerical integration method. The integrator sampling time is selected by measuring the time difference between consecutive applications of the controls $\mathbf{\Force}$ and $\mathbf{\Moment}$ to the plant. Figure \ref{fig:states_comparison_validation} presents the comparison between $\mathbf{x}$ and $\mathbf{x}_{sim}$ for a simulation where $\mathbf{\Force}\left[N\right] = \left[1.0,\,0.02,\,0.04\right]$ and $\mathbf{\Moment}\left[N\,\cdot\,m\right] = \left[-0.03,\,0.02,\,0.2\right]$. The average integrator sampling time is $2.39\,\text{ms}$.

\begin{figure}[!htb]
  \centering
  \includegraphics[width=1.0\textwidth]{Figures/states_comparison_3d_sim.pdf}
  \caption[Comparison between simulated and propagated states on a free floating environment.]{Comparison between simulated and propagated states on a free floating environment.}
  \label{fig:states_comparison_validation}
\end{figure}

In figure \ref{fig:states_comparison_validation}, $\mathbf{x}_{sim}$ is on blue, whereas $\mathbf{x}$ is on red. It is clear that \ref{eq:fullStateSpace} reliably simulates a rigid body floating in space. As the simulation progresses, the error between $\mathbf{x}_{sim}$ and $\mathbf{x}$ piles up and a more visible deviation is observed, which is normal. Therefore, if one neglects unmodeled wheel-terrain contact features, \ref{eq:fullStateSpace} is valid for an all-terrain simulation of a rigid wheeled vehicle. Take notice of the fact that the graph relative to $\yaw$ does not present any remarkable deviation between $\yaw$ and $\yaw_{sim}$. Instead, $\yaw_{sim}$ goes from $\pi$ to $-\pi$ due to the angle discontinuity as it is recorded from the simulation.

For the sake of completeness, the comparison between $\mathbf{x}_{sim}$ and $\mathbf{x}$ is presented in figure \ref{fig:states_comparison_validation_contacts}, when the vehicle is subject to wheel-terrain contacts. 

\begin{figure}[!htb]
  \centering
  \includegraphics[width=1.0\textwidth]{Figures/states_comparison_3d_sim_contacts.pdf}
  \caption[Comparison between simulated and propagated states on an all-terrain environment.]{Comparison between simulated and propagated states on an all-terrain environment.}
  \label{fig:states_comparison_validation_contacts}
\end{figure}

In this case, the results do not ensure a clear validation of \ref{eq:fullStateSpace}. This is due to unmodeled damping and ground penetration effects from the \acrshort{ode} physics engine of \textsc{Gazebo}, as well as error accumulation. However, in the first ten seconds of the simulation, the results present similarities, although it gets unstable after this period. So, \ref{eq:fullStateSpace} presents some reliability to model an all-terrain motion of a vehicle rigid body. 

\subsection{Maximum inclination}

There is also a need to check to what extent \acrshort{ode} models the Coulomb friction model. In order to carry this validation, a wheeled vehicle model is placed on ramps of increasing slope. On each of these ramps, the wheels are torque-actuated to the highest possible traction force. Then, if the vehicle is unable to traverse some ramp, it means the traction force has reached the border of the Coulomb friction cone and is not able to counter the weight component on $\BodyFrame_x$. The respective ramp slope is the highest admissable slope the vehicle can go through, with respect to the selected friction coefficient $\FrictionCoefficient$.

Then, these tests are carried out for several friction coefficients and the results are presented in figure \ref{fig:states_comparison_validation_contacts}.
...
\begin{minipage}{0.45\linewidth}
  	...
  	\begin{axis}[
		...
		% Plot theoretical maximum admissable slope
    	\addplot[red, thick] {atan(x) * pi / 180};
	    \addlegendentry{$f(\mu) = \arctan(\mu)$}
	    
	    % Plot maximum admissable slope best fit
    	\addplot[blue, thick] {atan(0.699742 * x) * pi / 180};
	    \addlegendentry{$f(\mu) = \arctan(k_{opt}\,\times\,\mu)$}
	    
	    \addplot[only marks, mark=*] table[x=x, y=y] {Data/maxInclination.dat};
    	\addlegendentry{Maximum achievable slope}
 	\end{axis}
\end{tikzpicture}
\end{minipage}
\begin{minipage}{0.45\linewidth}
    ...
    \end{tabular}
\end{minipage}
\caption[Comparison between simulated and propagated states on an all-terrain environment.]{Comparison between simulated and propagated states on an all-terrain environment.}			\label{fig:states_comparison_validation_contacts}
\end{figure}

After $\mu = 0.7$ it was not possible to collect more data, because on high friction coefficients, more traction force is available, which makes the vehicle more prone to overcome very high slopes. However, at high slopes, the vehicles easily tips-over, which makes it difficult to reasonably assess the maximum achievable slope. Even though, the collected data points already allow to estimate a relationship between $\mu$ and $f(\mu)$. This estimation is obtained by computing the best fit of an $\arctan(\cdot)$ like function to the data points.

\begin{equation}
	\underset{k}{\text{minimize}} \hspace{1mm} \sum_{i=0}^{7} \left(\arctan(k \, \times \, \mu_i) - \pitch_i\right)^2
	\label{eq:optimizeMaxAchievableSlope}
\end{equation}

Equation \ref{eq:optimizeMaxAchievableSlope} is the optimization problem that aims to provide $k_{opt}$. It is implemented in \acrshort{casadi} and \acrshort{ipopt} is the solver. Its solution yields $k_{opt} = 0.699742$. 

This result will be integrated into a future insertion of the Coulomb friction model on a \acrshort{nlp}. Moreover, one will attempt that the vehicle stays below the maximum achievable slope, with respect to the coefficient of friction in play.