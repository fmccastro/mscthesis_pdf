...
In this thesis, it is important to retrieve each wheel's contact point with the terrain it is touching on, in order to enable the transfer of forces applied at $\contactPoint_i$ to frame $\BodyFrame$ and to describe the velocity of each wheel with respect to a frame $\WheelFrame_i$ with axes $\WheelFrame^i_x$ and $\WheelFrame^i_y$ tangent to the terrain at $\contactPoint_i$.
...
\begin{assumption}
It is assumed $\FrictionCoefficient_x = \FrictionCoefficient_y$.
...
\begin{equation}
	\mathbf{\ForceSubscript{\FrictionSubscript}} = \sum_{i=1}^{4} \mathbf{\RotMatrix}_{\WheelFrame^i}^{\BodyFrame} \left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)_i \quad \mathbf{\Moment}_{\left( \mathbf{\ForceSubscript{\FrictionSubscript}} \right)} = 0
...
\begin{equation}
	\mathbf{\Moment}_{\left( \mathbf{\ForceSubscript{\NormalForceSubscript}} \right)} = \mathbf{\Moment}_{\left( \mathbf{\ForceSubscript{\gravity}} \right)} = \mathbf{0}
...
\mathbf{\Position}^T & \bm{\Orientation}^T & \mathbf{\LinVelocity}^T & \bm{\AngVelocity}^T & \bm{\AngVelocity}_i^T
\end{bmatrix}^T$.

\begin{equation}
	\mathbf{f}\left( \mathbf{x} \right) = \begin{cases}
		\DerMathBold{\Position} = \mathbf{\RotMatrix}_{\BodyFrame}^{\WorldFrame} \mathbf{\LinVelocity}\\
		...
		\bm{\dot{\AngVelocity}}_i = \mathbf{\InertiaTensor}_{\text{wheel}}^{-1} \bm{\Torque}_i 
	\end{cases}
...
\mathbf{A}_1 & \mathbf{A}_2 & \mathbf{A}_3 & \mathbf{A}_4 & \mathbf{\Force_{\FrictionSubscript}} & \mathbf{\Force_{\gravity}} & \mathbf{\Force_{\NormalForceSubscript}} \\
\mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_1 \right)  \mathbf{A}_1 & \mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_2 \right)  \mathbf{A}_2 & \mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_3 \right) \mathbf{A}_3 & \mathbf{\SkewSymmetricMatrix} \left( \mathbf{\contactPoint}_4 \right) \mathbf{A}_4 & \mathbf{0} & \mathbf{0} & \mathbf{0} 
\end{bmatrix} \begin{bmatrix}
\bm{\Torque}_1 \\
\bm{\Torque}_2 \\
\bm{\Torque}_3 \\
