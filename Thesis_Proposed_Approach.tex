...
The mobile robot position and orientation with respect to frame $\WorldFrame$ is described as $\mathbf{\Position} = [x\;  y\; z]^T$ and $\bm{\Orientation} = [\roll\; \pitch\; \yaw]^T$, respectively. On the other hand, the mobile robot linear and angular velocities with respect to frame $\BodyFrame$ stands as $\mathbf{\LinVelocity} = [ \vx \; \vy \; \vz ]^T$ and $\bm{\AngVelocity} = [\wx \; \wy \; \wz]^T$, respectively. Then, it is possible to map the mobile robot $\Robot$ velocity between frames $\BodyFrame$ and $\WorldFrame$ by taking a Yaw-Pitch-Roll intrinsic rotation convention of euler angles, with each elemental rotation matrix denoted in \ref{section:rotationMatrix}.
...
Secondly, a mapping between Euler angle rates $\DerBold{\Orientation}$ and $\bm{\AngVelocity}$ is derived by tranforming each Euler rotation rate from its intermediate frame to body frame $\BodyFrame$.
...
\begin{equation}
	...
	\bm{\AngVelocity} &= \begin{bmatrix}
		...
		0\\
		0
	\end{bmatrix} + \mathbf{\RotMatrix}_x(\roll)
	\begin{bmatrix}
		0\\
		\dot{\pitch}\\
		0
	\end{bmatrix} + \mathbf{\RotMatrix}_x(\roll) \mathbf{\RotMatrix}_y(\pitch)
	\begin{bmatrix}
		0\\
		0\\
		\dot{\yaw}
	...
	\begin{bmatrix}
		1 & 0 & \sin(\theta) \\
		0 & \cos(\roll) & -\sin(\roll) \cos(\theta) \\
		0 & \sin(\roll) & \cos(\roll) \cos(\pitch)
	\end{bmatrix} \DerBold{\Orientation}
	\end{aligned}
	\label{eq:rotationalFreeBody}
\end{equation}

The inverse of the rotation matrix presented in \ref{eq:rotationalFreeBody} provides a mapping between angular velocities from frame $\BodyFrame$ to $\WorldFrame$.

\begin{equation}
	\DerBold{\Orientation} = 
	\begin{bmatrix}
		1 & \tan(\pitch) \sin(\roll) & -\tan(\pitch) \cos(\roll)\\
		0 & \cos(\roll) & \sin(\roll)\\
		0 & -\frac{\sin(\roll)}{\cos(\pitch)} & \frac{\cos(\roll)}{\cos(\pitch)}
	\end{bmatrix} \bm{\AngVelocity} = \mathbf{\bar{\RotMatrix}}_{\BodyFrame}^{\WorldFrame} (\yaw, \pitch, \roll) \bm{\AngVelocity}
	\label{eq:rotationalFreeBodyInv}
\end{equation}

\subsection{Wheel kinematics}
\label{subsection:wheelKinematics}

A kinematics model is not only described by its free-body motion equations \ref{eq:translationFreeBody} and \ref{eq:rotationalFreeBodyInv} but also by geometrical constraints \cite{Kozlowski2004}. Each wheel is actuated by a torque $\Torque_i$ which induces on it a spin rate, $\mathbf{\dot{\wheelAngPosition}}_i,\, i = 1,...,\,N_\WheelFrame$, where $N_\WheelFrame = 4$ for the cases presented in this thesis. 
...
Due to the fact that wheels are fixed, the mobile robot holds two controllable \acrshort{dof}: translation along its longitudinal direction $\BodyFrame_x$, and rotation around its vertical axis $\BodyFrame_z$. Therefore, lateral skidding from the vehicle is expected to remain close to zero on flat terrain, although its wheels are prone to skidding when the mobile robot finds itself on uneven or inclined surfaces (figure \ref{fig:wheelForcesMoments}). If the mobile robot follows a straight line path, its wheels velocities are tangent to the current path. Moreover, the velocity vector of each wheel is tangent to the terrain that wheel is touching on. Under the smooth terrain assumption (assumption \ref{assumption:smoothTerrain}), it is considered that the mobile robot stands at a planar surface and the terrain profile local variation is smooth.

\begin{assumption}
	Smooth terrain assumption: the local terrain profile variation is smooth, which makes $\{\vz, \wx, \wy \} \approx 0$. At every iteration, it is considered that a robot stands at a planar surface and the wheel velocities equal the mobile robot velocity.
	\label{assumption:smoothTerrain}
\end{assumption}

In case assumption \ref{assumption:smoothTerrain} is discarded, these variables behave in such a way that the traversed terrain profile impose $\wx$ and $\wy$. On the other hand, the existence of holes or bumps force $\vz$ to be different from zero. $\LinVelocity_y$ rises above zero in case of lateral skidding.
...
\begin{equation}
	\mathbf{\LinVelocity}_i =
	\left[ \begin{array}{c;{2pt/2pt}c}
		\mathbf{\IdentityMatrix}_{3 \times 3} & 
		\begin{matrix}
			0 & -\wheelPosition_{i, z} & \wheelPosition_{i, y} \\
			\wheelPosition_{i, z} & 0 & -\wheelPosition_{i, x} \\
			-\wheelPosition_{i, y} & \wheelPosition_{i, x} & 0
		\end{matrix}
	\end{array} \right]
	\begin{bmatrix}
		\mathbf{\LinVelocity} \\
		\bm{\AngVelocity}
	\end{bmatrix} = \mathbf{\LinVelocity} + \bm{\omega} \times \mathbf{\wheelPosition}_i
	\label{eq:wheelVel}
\end{equation}
...
The presented wheeled mobile robot model contains four controllable joints at each wheel \acrshort{com}, which are typically located at each wheel's center. The mobile robot may roll, pitch or fall vertically with respect to frame $\BodyFrame$, because it will be moving through irregular terrain, which changes the balance of forces the mobile robot is subject to. Accordingly, dynamics is only controllable in the longitudinal, lateral or yaw directions of frame $\BodyFrame$. Moreover, external forces (e.g. wheel-terrain traction or wind) and gravity are also taken into account in this general model.
